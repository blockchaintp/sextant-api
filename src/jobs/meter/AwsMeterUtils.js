...
const sendCommandOrFail = async (client, command, iamRequired) => {
  try {
    ...
    logger.error({ error }, 'meter command failed');
    ...
    switch (error.name) {
      ...
      case 'InternalServiceErrorException':
        logger.warn({ error }, 'meter command failed unexpectedly.'
      ...
      case 'PlatformNotSupportedException':
        shutdownProcess('This container is not supported on the underlying platform.'
          + 'Currently, Amazon ECS, Amazon EKS, and AWS Fargate are supported.');
        break;
      case 'AccessDeniedException':
        shutdownProcess('AccessDeniedException because no identity-based policy allows'
          + ` the ${iamRequired} action`)
        break;
      default:
